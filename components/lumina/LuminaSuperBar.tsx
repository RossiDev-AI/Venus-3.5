...
import { 
    ...
    Camera, Zap, FlaskConical, Shapes, Monitor, 
    Activity, Sparkles, Wand2, Eye,
    Aperture, Focus, Filter, Workflow
} from 'lucide-react';
...
export const LuminaSuperBar = ({ onImportClick }: { onImportClick: () => void }) => {
    ...
    const handleToolAction = (item: any) => {
        ...
        selected.forEach(shape => {
            if (shape.type === 'lumina-image') {
                // Mescla os novos parâmetros com os atuais do shape
    ...
    return (
        <div className="w-full h-full flex items-center justify-between px-2 md:px-6 bg-[#0e0e11] relative select-none">
            ...
            {/* COLLECTIONS MENU - Scrollable on Mobile */}
