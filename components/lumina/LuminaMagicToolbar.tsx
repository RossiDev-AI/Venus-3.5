...
import { Sparkles, SlidersHorizontal, Wand2, Eraser, Zap, Move3d, Layers } from 'lucide-react';
...
const LuminaMagicToolbar: React.FC<LuminaMagicToolbarProps> = ({ onTransform, isProcessing, onRemoveBG, props, onUpdate }) => {
      ...
      >
        <div className="bg-[#0e0e11]/95 backdrop-blur-2xl border border-white/10 p-1 rounded-[2.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.5)] flex flex-col gap-2">
          ...
          <div className="flex bg-black/40 p-1.5 rounded-[2rem] border border-white/5 mx-4 mt-4">
             <button onClick={() => setActiveTab('gen')} className={`flex-1 flex items-center justify-center gap-2 text-[9px] font-black uppercase py-2.5 rounded-full transition-all ${activeTab === 'gen' ? 'bg-indigo-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}><Sparkles size={12}/> Neural</button>
             <button onClick={() => setActiveTab('adjust')} className={`flex-1 flex items-center justify-center gap-2 text-[9px] font-black uppercase py-2.5 rounded-full transition-all ${activeTab === 'adjust' ? 'bg-indigo-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}><SlidersHorizontal size={12}/> Optics</button>
             <button onClick={() => setActiveTab('fx')} className={`flex-1 flex items-center justify-center gap-2 text-[9px] font-black uppercase py-2.5 rounded-full transition-all ${activeTab === 'fx' ? 'bg-indigo-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}><Zap size={12}/> FX</button>
          </div>
          ...
          <div className="p-6 min-h-[220px]">
            ...
            {activeTab === 'adjust' && (
                <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                   {[
                       { k: 'brightness', l: 'Exposure', min: 0, max: 2, def: 1 },
                       { k: 'contrast', l: 'Contrast', min: 0, max: 2, def: 1 },
                       { k: 'saturation', l: 'Saturation', min: 0, max: 2, def: 1 },
                       { k: 'face_radius', l: 'Focus Radius', min: 0, max: 1, def: 0.2 }
                   ].map(s => (
            ...
            {activeTab === 'fx' && (
                <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                   {[
                       { k: 'blur', l: 'Lens Blur', min: 0, max: 10, def: 0 },
                       { k: 'grain', l: 'Film Grain', min: 0, max: 1, def: 0 },
                       { k: 'bokeh', l: 'Depth Bokeh', min: 0, max: 1, def: 0 },
                       { k: 'depthStrength', l: 'Depth Map Force', min: 0, max: 50, def: 20 }
                   ].map(s => (
                       <div key={s.k} className="space-y-1.5">
                          <div className="flex justify-between text-[8px] font-black text-zinc-500 uppercase px-1">
                            <span>{s.l}</span>
